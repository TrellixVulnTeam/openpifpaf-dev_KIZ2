...
from .constants import LANE_KEYPOINTS, LANE_SKELETON, HFLIP, \
    LANE_SIGMAS, LANE_POSE, LANE_CATEGORIES, LANE_SCORE_WEIGHTS


class LaneKp(DataModule):
    """
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module Lane')

        group.add_argument('--lane-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--lane-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--lane-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--lane-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--lane-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        assert not cls.extended_scale
        group.add_argument('--lane-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--lane-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--lane-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--lane-no-augmentation',
                           dest='lane_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--lane-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')
        group.add_argument('--lane-upsample',
                           default=cls.upsample_stride, type=int,
                           help='head upsample stride')
        group.add_argument('--lane-min-kp-anns',
                           default=cls.min_kp_anns, type=int,
                           help='filter images with fewer keypoint annotations')
        group.add_argument('--lane-bmin',
        ...
        eval_set_group.add_argument('--lane-eval-test2017', default=False, action='store_true')
        eval_set_group.add_argument('--lane-eval-testdev2017', default=False, action='store_true')

        group.add_argument('--lane-no-eval-annotation-filter',
                           dest='lane_eval_annotation_filter',
                           default=True, action='store_false')
        group.add_argument('--lane-eval-long-edge', default=cls.eval_long_edge, type=int,
                           dest='lane_eval_long_edge', help='set to zero to deactivate rescaling')
        assert not cls.eval_extended_scale
        group.add_argument('--lane-eval-extended-scale', default=False, action='store_true',
                           dest='lane_eval_extended_scale',)
        group.add_argument('--lane-eval-orientation-invariant',
                           default=cls.eval_orientation_invariant, type=float,
                           dest='lane_eval_orientation_invariant')
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # Lane specific
        cls.train_annotations = args.lane_train_annotations
        cls.val_annotations = args.lane_val_annotations
        cls.train_image_dir = args.lane_train_image_dir
        cls.val_image_dir = args.lane_val_image_dir

        cls.square_edge = args.lane_square_edge
        cls.extended_scale = args.lane_extended_scale
        cls.orientation_invariant = args.lane_orientation_invariant
        cls.blur = args.lane_blur
        cls.augmentation = args.lane_augmentation  # loaded by the dest name
        cls.rescale_images = args.lane_rescale_images
        cls.upsample_stride = args.lane_upsample
        cls.min_kp_anns = args.lane_min_kp_anns
        cls.b_min = args.lane_bmin

        # evaluation
        cls.eval_annotation_filter = args.lane_eval_annotation_filter
        cls.eval_long_edge = args.lane_eval_long_edge
        cls.eval_orientation_invariant = args.lane_eval_orientation_invariant
        cls.eval_extended_scale = args.lane_eval_extended_scale

        if (args.lane_eval_test2017 or args.lane_eval_testdev2017) \
    ...
    def _preprocess(self):
        ...
        return transforms.Compose([
            ...
            transforms.RandomApply(
                transforms.HFlip(LANE_KEYPOINTS, HFLIP), 0.5),
            rescale_t,
            transforms.RandomApply(
        ...
        return transforms.Compose([
            *self.common_eval_preprocess(),
            transforms.ToAnnotations([
                ...
                transforms.ToCrowdAnnotations(LANE_CATEGORIES),
            ]),
            transforms.EVAL_TRANSFORM,
        ])
    ...
    def metrics(self):
        return [metric.Coco(
            ...
            category_ids=[1],
            iou_type='keypoints',
            keypoint_oks_sigmas=LANE_SIGMAS,
        )]
